...
int16_t GUI::scrollingMainMenu()
{
	...
	uint8_t cursorX = 0;
	uint8_t cursorY = 0;
	uint8_t elements = 8; //5 default apps
	uint8_t x_elements = 3;
	uint8_t y_elements = round((elements/x_elements)+0.5);
	uint8_t pageNumber = elements / 6;
	uint8_t pageIndex = 0;
	uint8_t cameraY = 0;
	String appNames[] = {"Clock", "Calculator"};
	String passcode = "";
	uint32_t passcodeMillis = millis();
	long elapsedMillis = millis();
	long elapsedMillis2 = millis();
	...
	while (1)
	{
		...
		while(cursorY*y_elements + cursorX > elements - 1)
			cursorX--;
		index = cursorY * x_elements + cursorX;
		Serial.println(index);
		Serial.println(cameraY);
		Serial.println("-----------------");
		delay(5);
		mp.display.setTextFont(2);
		mp.display.setCursor(0,-2);
		...
		if(index < 6)
			...
			mp.display.print(appNames[index-6]);
		...
		if (mp.buttons.released(JOYSTICK_D)) //UP
		{
			...
			else if((pageIndex*2 + cameraY) % 2 == 1)
			{
				cursorY--;
				cameraY = 0;
			}
			else if((pageIndex*2 + cameraY) % 2 == 0 && pageIndex > 0)
			{
				cameraY = 0;
				cursorY--;
				pageIndex--;
		...
		if (mp.buttons.released(JOYSTICK_B))//DOWN
		{
			...
			else if((pageIndex*2 + cameraY) % 2 == 0)
			{
				cursorY++;
				cameraY++;
			}
			else if ((pageIndex*2 + cameraY) % 2 == 1 && pageIndex < pageNumber)
			{
				cameraY = 1;
				cursorY++;
				pageIndex++;
